...
export function detectBGcolor(logoSelector, type = 'logo') {
    ...
    // NOTE: this is only for one element at a time with one ref or selector
...
export function detectNavSpanColor(spanElement) {
    ...
    // NOTE: this is for multiple elements with same class or ref
...
export function detectNavImageColor(imageElement) {
    if (typeof window === 'undefined' || !imageElement) return;


    // NOTE: this is for multiple elements with same class or ref

    const elementId = imageElement.getAttribute('data-observer-id') ||
                        `span-${Math.random().toString(36).substr(2, 9)}`;
    imageElement.setAttribute('data-observer-id', elementId);
    ...
    function checkSpanBackground() {
        if (!imageElement || !document.body.contains(imageElement)) return;
        ...
        try {
            ...
            const elementBehind = elementsAtPoint.find(el => {
                // Skip the span itself and all of its parent elements
                let currentParent = imageElement;
                while (currentParent) {
                    if (el === currentParent) return false;
                    currentParent = currentParent.parentElement;
                }
            ...
            if (elementBehind) {
                ...
                imageElement.style.filter = isLightBg ? 'brightness(0) saturate(0%)' : '';
                // Apply color directly to the span
                imageElement.style.color = '';
            }
        } catch (error) {
            console.error('Error checking span background:', error);
        }
...
export function detectNavBGColor(BgElement) {
    if (typeof window === 'undefined' || !BgElement) return;

    // NOTE: this is for multiple elements with same class or ref

    const elementId = BgElement.getAttribute('data-observer-id') ||
                        `span-${Math.random().toString(36).substr(2, 9)}`;
    BgElement.setAttribute('data-observer-id', elementId);
    ...
    function checkSpanBackground() {
        if (!BgElement || !document.body.contains(BgElement)) return;
        ...
        try {
            ...
            if (elementBehind) {
                ...
                BgElement.style.background = isLightBg ? '#000000' : '#ffffff';
                // Apply color directly to the span
